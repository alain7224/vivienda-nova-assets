...
export const appRouter = router({
  ...
  admin: router({
    ...
    updateSettings: adminProcedure.input(settingsInput).mutation(async ({ input }) => { const { whatsappEnabled, whatsappAnimationEnabled, ...values } = input; await updateSiteSettings({ ...values, ...(whatsappEnabled === undefined ? {} : { whatsappEnabled: whatsappEnabled ? 1 : 0 }), ...(whatsappAnimationEnabled === undefined ? {} : { whatsappAnimationEnabled: whatsappAnimationEnabled ? 1 : 0 }) }); return { success: true }; }),
